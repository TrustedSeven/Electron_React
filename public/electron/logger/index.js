...
const fatal = (text) => {
  if (isDev) {
    console.log(
      `${colors.red} [${new Date().toLocaleString()}] - ${colors.white} ${text}`
    );
  } else {
    logs.push(`[${new Date().toLocaleString()}] - ${text}`);
  }
...
const success = (text) => {
  if (isDev) {
    console.log(
      `${colors.green} [${new Date().toLocaleString()}]-${colors.white} ${text}`
    );
  } else {
    logs.push(`[${new Date().toLocaleString()}] - ${text}`);
  }
...
module.exports = { warning, info, fatal, success, logString, clearLogs };
